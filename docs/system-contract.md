...
The system has three independently releasable projects:

1. **Environment Atlas** reconstructs environmental input products.
...
No project reaches into another project's internal orchestration. Integration is
...
| Name | Meaning | Owner |
| --- | --- | --- |
| `EmissionRelease` | immutable surface-emission manifests, dictionaries and chunks | Environment Atlas emission domain |
| `AtmosphereFieldRelease` | immutable 4-D environmental-state manifests, source runs and chunks | Environment Atlas atmosphere domain |
| `EnvironmentDisplayProduct` | deterministic map-oriented derivative of exactly one domain release | Environment Atlas producing domain |
| `SurfaceEmissionBatch` | contiguous result of one emission query | Environment Atlas format/provider boundary |
| `AtmosphereStateVolume` | contiguous regional 4-D result of one atmosphere query | Environment Atlas format/provider boundary |
...
| `EmissionFieldProvider` | Physics-side batch/query adapter over one `EmissionRelease` | Physics adapter; Environment Atlas defines producer semantics |
| `AtmosphereFieldProvider` | Physics-side batch/query adapter over one `AtmosphereFieldRelease` | Physics adapter; Environment Atlas defines producer semantics |
...
Environment Atlas decoding/query:
...
2. Environment Atlas resolves the selected emission and atmosphere products and
...
not change. Environment Atlas never propagates light. Physics never performs raw
provider ingest/fusion. Viewer never invents physics or feeds display tiles back
into Physics.

Environment Atlas may retain terrain, land/sea, snow, roughness and surface
