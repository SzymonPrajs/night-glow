...
Emission-chunk intensity bounds are source-side only. The solver combines them with atmosphere-field uncertainty and a conservative transfer bound to control outward source and atmospheric-domain expansion; it never interprets `J_DNB` bounds as observer radiance or adopts a fixed atlas radius.
