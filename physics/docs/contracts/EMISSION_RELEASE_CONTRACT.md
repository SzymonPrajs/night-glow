# Environment Atlas emission domain consumer contract

Physics consumes Environment Atlas emission domain as an independent, versioned source product. It does not merge the atlas workspace, repeat satellite/source inference, or require the atlas to emit Physics' internal spectral grid. The atlas owns the normative producer semantics; this document owns the Physics adapter and consumer obligations.

The sibling atlas design describes its producer contract in [Environment Atlas emission domain: Physics handoff contract](../../../environment-atlas/docs/18-physics-handoff.md). If the projects are developed in separate repositories, the Physics dependency pins an atlas schema/release version and carries the corresponding conformance fixture rather than relying on a relative source-tree link.

## 1. Shared language

| Atlas term | Physics meaning |
| --- | --- |
...
| `TemporalProfile` | Optional factor normalized to 1 at the atlas reference phase |
...
Environment Atlas emission domain owns raw nighttime radiometry, atmospheric/view correction of the satellite observation, exact support integration, conservative H3 refinement, source-evidence fusion, source profiles and their evidence, source-side uncertainty, release format, provenance, licence partitions, and chunk intensity bounds.
...
Physics may depend on released atlas schema/format crates or an independent conforming decoder. Environment Atlas emission domain never depends on Physics crates.

## 3. Adapter input

The adapter consumes release/dictionary/chunk descriptors containing:

- atlas schema/model/release identities and content hashes;
...
Height/elevation states declare datum and distinguish surface elevation, source height above surface, and ellipsoidal height. Physics combines atlas source-height evidence with its terrain data exactly once.
...
6. Project the resolved spectrum from vacuum-nanometre atlas support into the chosen Physics wavelength basis using a versioned operator and measured projection error.
...
The atlas value is already an outgoing surface-side signal and may combine direct upward and first-reflected lamp light. Physics must not apply the ground BRDF to the initial atlas value again. If the atlas explicitly separates direct and first-reflected components, Physics may consume them as separate outgoing components without recomputing their pre-atlas history.
...
Likewise, the atlas has already reconstructed a corrected source-side DNB signal from the satellite path. Physics propagates a new source-to-observer path and never reapplies or reverses the satellite correction.
...
Physics does not make the atlas astronomy-dependent, and the atlas does not duplicate SOFA/JPL/time-scale logic. Missing profile inputs preserve the reference factor and mark unresolved; longitude is not a time zone.
...
The solver may run a central value, interval, sensitivity sweep, or ensemble. Any component it does not propagate is listed as omitted, not silently absorbed into numerical residual. Atlas uncertainty, Physics model uncertainty, and numerical convergence residual remain separate fields.
...
Both projects use the same tiny open conformance fixture, owned and versioned by Environment Atlas emission domain. Physics tests:
...
- deterministic cache invalidation across atlas/profile revisions.

End-to-end validation pins both `emission_release_id` and `physics_model_revision`. Residual reports separate atlas source reconstruction, profile/scenario uncertainty, atmosphere/terrain inputs, Physics model inadequacy, numerical error, and display differences.
