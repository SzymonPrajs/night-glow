...
The atlas supplies only source-side `J_DNB` and uncertainty bounds. Physics combines those with spectral/profile uncertainty and a conservative transfer bound; only Physics can decide that the unprocessed observer-radiance tail is negligible.
