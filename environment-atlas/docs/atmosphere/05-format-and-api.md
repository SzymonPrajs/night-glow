...
The Environment Atlas atmosphere Wasm decoder validates releases, selects chunks, decodes arrays, transforms vertical coordinates and returns coarse transferable buffers. It may live in the same coordinator worker as Physics but retains its own schema/version. It never performs one JS call per voxel.
