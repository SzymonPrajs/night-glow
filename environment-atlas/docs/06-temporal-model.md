...
4. Estimate factors relative to the atlas reference phase.
...
The query accepts an explicit `EmissionTimeContext`. Physics/astronomy supplies UTC and solar/ephemeris fields; the application supplies authoritative IANA civil-zone, time-zone-database revision, and policy/holiday context. Environment Atlas emission domain evaluates the stored profile but does not implement leap-second conversion, ephemerides, time-zone discovery, or solar geometry. This keeps the projects independent and prevents two astronomy implementations.
