...
### F. Environment emission domain boundary conformance
...
Explicit negative tests prove that Physics does not replay the satellite-path correction, multiply by `pi` without a named scenario, invent a spectrum/angular/time profile, use H3 averages as physical geometry, or apply the ground BRDF to the initial outgoing Environment value.

### G. Environment atmosphere boundary conformance
