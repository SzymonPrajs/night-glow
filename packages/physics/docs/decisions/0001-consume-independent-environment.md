# ADR 0001: Consume Environment emission domain as an independent product
...
Environment emission domain and Physics are separate expert projects. Environment reconstructs source-side artificial emission from heterogeneous observations. Physics propagates source boundary conditions through terrain, atmosphere, surface coupling, observation, and rendering. Merging their workspaces or duplicating inference would couple release cycles, invite double correction, and make errors difficult to attribute.

Environment's defensible baseline is `J_DNB [W sr^-1]`, a DNB-response directional intensity integrated over exact surface support for a declared corrected reference view. Spectrum, upward angle, and time can remain unresolved. Earlier Physics wording expected generic spectral intensity or total flux and therefore over-specified information Environment may not possess.

## Decision

Physics consumes immutable Environment emission domain releases through a one-way, versioned product contract. Environment emission domain owns schema/format meanings and the conformance fixture. Physics implements `EmissionFieldProvider` using released Environment schema/format crates or an independent conforming decoder.
...
- Environment emission domain does not adopt Physics' runtime spectral grid or atmospheric solver.
...
- The already-outgoing Environment value does not receive the initial ground BRDF twice; later atmosphere–surface orders remain Physics work.
...
The detailed consumer obligations are in [the Environment emission domain contract](../contracts/emission-release.md).
