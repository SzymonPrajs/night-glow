...
Spectrum, upward angle, and time are independent tagged profiles and may remain unresolved. Environment proposes a higher-resolution source spectrum (10 nm bins from 350–950 nm plus narrow lines), which Physics will project into its own eventual runtime basis.

## What Physics does not research again

Environment owns Black Marble product selection and QA; satellite-path atmospheric/view/lunar/BRDF correction semantics; exact support integration and conservative H3 refinement; OSM/WSF/GHSL/inventory fusion; source spectral/angular/temporal evidence; source-side uncertainty; source-only validation; release format; provenance and licence partitions.

Physics reviews those outputs for compatibility but does not build a second satellite inversion model inside its precompute or Wasm solver. Detailed Environment research remains in its own project.
...
- combine Environment source-height evidence with Physics terrain exactly once;
- preserve coverage/censoring, uncertainty components, evidence, provenance, licence, and attribution;
- keep source/profile/Environment revisions distinct from propagation/scenario revisions;
...
The complete Physics-side contract is [Environment emission domain consumer contract](../contracts/emission-release.md). Environment owns the producer meanings and future machine-readable schema; Physics either consumes its released schema/format decoder crates or proves an independent decoder against the same conformance fixture.
