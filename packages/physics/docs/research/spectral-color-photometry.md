...
spectral radiance
 ...
 -> physical observer/sensor response
 -> linear display primaries
 -> Viewer exposure/white balance/gamut mapping/tone mapping
 -> output transfer function
```

The physical field remains available for metrics after display mapping. If low-light desaturation or visual adaptation is treated as a physical observer model, it has an explicit `observer_response_id` and revision; ordinary presentation exposure/tone mapping remains Viewer-owned and never edits the atmosphere or Physics result.
