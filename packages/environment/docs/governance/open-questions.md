...
| What exact schema/decoder dependency will Physics use? | Benchmark a published `environment-schema`/`environment-format` dependency against an independently conforming decoder in native and Wasm builds | One-way dependency decision with the same conformance fixture and no ingestion/solver coupling |
...
- Environment atmospheric **state** and Physics atmospheric **optics/propagation** are separate products; see [ADR 0003](../decisions/0003-environment.md).
