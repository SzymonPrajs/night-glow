...
The final surface-emission product uses spatial cell records with shared profile dictionaries. The conceptual record is:
...
Gas flares, offshore platforms, fishing fleets, fires, and temporary events do not fit settlement geometry. They require separate source classes and temporal behavior. Stable industrial/offshore sources may remain in annual releases with their actual spatial support; fires and short events belong in optional temporal deltas. The global model must not mask every non-OSM light simply because no road/building explains it.

## 4.8 Consumer-facing semantic states

The release schema exposes profile and coverage states as tagged values rather than sentinel IDs:

```text
ProfileState<T> = resolved(T) | unresolved
CoverageStatus = supported_emission | supported_dark_or_upper_bound | no_evidence
```

These states survive native/Wasm decoding and Physics adaptation. `unresolved` does not mean a zero profile, `no_evidence` does not mean darkness, and `supported_dark_or_upper_bound` retains its censoring/detection semantics.

The complete Physics handoff record and canonical terms are defined in [physics-handoff.md](physics-handoff.md). In particular, `J_DNB` remains unchanged at the boundary; source wavelength/direction/time resolution is a separate, evidence-dependent conversion.
