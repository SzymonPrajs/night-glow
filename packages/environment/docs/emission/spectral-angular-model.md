...
Wavelengths at the consumer boundary are vacuum nanometres. Environment retains this high-resolution source representation; Physics projects it into its independently selected runtime basis with a versioned, error-tested operator. Environment is never required to emit the current application's eight bands.
...
Directions use a source-local right-handed east–north–up frame: `theta=0` is local up, `theta=90°` is the horizon, and absolute `phi=0` is true geodetic north increasing eastward. Feature-relative harmonics declare their orientation separately. The profile stores its normalization kind; normalization against the DNB reference direction must not be confused with unit hemispheric integral.
...
This same forward closure is a consumer invariant: a compatible spectrum/angular family evaluated through the stored DNB response at the corrected reference view must reproduce time-adjusted `J_DNB` within declared uncertainty. If either profile is unresolved, Physics must use insufficient-evidence, bounds, or an explicit scenario behavior rather than a hidden default.
