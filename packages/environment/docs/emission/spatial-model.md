...
H3 is not the physical Earth model. The Physics handoff uses WGS84 geodetic support, exact declared areas, explicit height/datum metadata, and source-local east–north–up directions. H3 average cell centres/areas are never substituted into propagation geometry.
...
Environment supplies only source-side `J_DNB` and uncertainty bounds. Physics combines those with spectral/profile uncertainty and a conservative transfer bound; only Physics can decide that the unprocessed observer-radiance tail is negligible.
