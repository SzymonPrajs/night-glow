...
2. **Source reconstruction:** Environment predicts withheld fine nighttime radiance/inventories.
3. **Propagation physics:** a known source field and atmosphere predict measured sky radiance.
4. **End to end:** the public-data emission release plus atmosphere predicts ground observations.
...
## Physics-boundary conformance

Before Phase-9 integration, Environment publishes the tiny open fixture in [physics-handoff.md](physics-handoff.md). Native and Wasm tests must agree on `J_DNB`, exact support/hierarchy, corrected reference view, profile normalization/state, time context, uncertainty, provenance, and corruption rejection.

The independent Physics suite consumes the same fixture and proves that it does not change `J_DNB` before propagation, replay the satellite-path correction, invent unresolved profiles, apply the ground BRDF to the initial outgoing emission value, lose missing/quality states, or use H3 averages as physical geometry. End-to-end fixtures pin emission, atmosphere and Physics model revisions so residual attribution remains possible.
