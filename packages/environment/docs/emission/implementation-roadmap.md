...
Run the complete open-data pipeline twice from a frozen recipe. Produce the emission release, provenance, attribution, QA/coverage, compute report, and source-only validation.
...
Freeze schema v1 from the already reviewed handoff, publish native/Wasm conformance fixtures, implement chunk streaming/tail bounds, and validate at paired ground/atmosphere sites. Keep solver changes outside this project's ingestion crates and keep Environment ingestion/fusion code outside Physics.
