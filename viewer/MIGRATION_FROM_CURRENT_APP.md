...
- current source, eight weather presets and sky models are useful fixtures but are not the proposed Environment Atlas/Physics package boundaries.
...
- use fixture adapters before production Environment Atlas/Physics packages exist;
...
| physical glow field | placeholder/conditional | Physics + Environment Atlas | reference/convergence tests | new model accepted |
| eight weather presets | fixture only | Environment Atlas state + Physics optics | profile/column/volume and radiance cases | `standard_scenario` semantics accepted |
