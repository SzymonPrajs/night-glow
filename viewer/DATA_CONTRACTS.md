...
The Viewer depends on products, not source code. Environment Atlas and Physics remain independently versioned Rust/Wasm projects. Inside Environment Atlas, emission and atmosphere are also independently versioned domains. Every crossing includes units, reference frame, spatial/vertical/time support, schema identity, `DataValidity`, domain status, evidence, uncertainty, provenance and—only for computed outputs—fidelity/convergence.

The repository [unified system contract](../docs/system-contract.md) is normative for all capitalized product/type names, revision/time fields, atmosphere selection/evidence values and validity/runtime-state enums used below.
...
## 2. Environment Atlas → Globe contract

Each Environment Atlas domain owns its authoritative release and conformance fixture. A separate deterministic display-build tool may derive an `EnvironmentDisplayProduct`, but its manifest must include:
...
## 3. Environment Atlas → Physics contracts
...
See the Physics [Environment contract](../physics/docs/contracts/ENVIRONMENT_ATLAS_CONTRACT.md), detailed [emission contract](../physics/docs/contracts/EMISSION_RELEASE_CONTRACT.md), and Atlas producer handoffs for [emission](../environment-atlas/docs/18-physics-handoff.md) and [atmosphere](../environment-atlas/docs/21-atmosphere-physics-handoff.md).
...
Pollution layers use a broader Viewer-owned manifest because they need not share Atlas physics:
