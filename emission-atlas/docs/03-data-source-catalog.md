...
## D. Atmosphere and terrain inputs for the future solver

These are catalogued here to keep the interface aligned, but are not embedded in the emission atlas.
...
The solver needs wavelength-dependent aerosol optical properties and vertical distribution, not just a single visibility number. Dataset selection must be validated separately from source reconstruction.
