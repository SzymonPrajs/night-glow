...
- target edition and reference year;
...
The first edition uses the latest **complete** calendar year shared by the required source products at recipe freeze time. It is not hardcoded in the design.
...
- machine-readable changes from the prior edition;
