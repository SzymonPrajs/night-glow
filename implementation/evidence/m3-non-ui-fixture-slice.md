...
| Physics native boundary | `cargo test --manifest-path packages/physics/Cargo.toml` | 17 tests pass across typed scenarios/errors, astronomy, data, the independent Environment adapter, transfer functions, external-reference validation, DAG scheduling/cancellation and Wasm output |
| Physics reference solve | `cargo run --release --manifest-path packages/physics/Cargo.toml -p nightglow-validation` | 24 coherent `f32` values match the language-neutral fixture with maximum relative error `4.094373956018e-8`; fine integration error is `2.793967610559e-7`; 3 libRadtran DISORT pure-absorption cases agree within `6.642263376598e-8` |
