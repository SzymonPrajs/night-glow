...
| native/Wasm boundary | `make wasm-probe` | Environment/Physics modules are 25,887/206,398 bytes, each starts at 1,114,112 bytes, scalar JS/Wasm drift is 0, and coherent-product relative error is `4.0944e-8` |
| non-UI coordinator | `make coordinator-test` | 7 tests pass against both Wasm modules: coherent product, cancellation, stale revision, unit drift, resource budget, success/failure buffer release, and stable memory across 100 scenarios |
