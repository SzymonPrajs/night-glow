...
| native/Wasm boundary | `make wasm-probe` | Environment/Physics modules are 25,887/317,241 bytes, each starts at 1,114,112 bytes; the Physics Wasm adapter independently decodes the full shared atmosphere fixture to 99,850 Pa, scalar JS/Wasm drift is 0, and coherent-product relative error is `4.0944e-8` |
| non-UI coordinator | `make coordinator-test` | 11 tests pass against both Wasm modules and the worker adapter: coherent product, cancellation, stale revision, unit drift, resource budget, runtime-manifest/Physics-identity rejection, success/failure buffer release, stable memory across 100 scenarios, complete runtime disposal during pending work, and worker disposal acknowledgement |
