...
| native reference kernel | `cargo run --release --manifest-path packages/physics/Cargo.toml -p nightglow-validation` | fine exponential-column integration error `2.793967610559e-7`; spherical-horizon optical depth `33.969905248062` converges within `3.9742e-15`; 3 libRadtran DISORT pure-absorption cases agree within `6.6423e-8`; 22 µs on the recorded local run |
